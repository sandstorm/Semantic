 ...
 * @covers F3\Semantic\Domain\Model\Rdf\Concept\Triple
 */
class TripleTest extends \F3\FLOW3\Tests\UnitTestCase {
	...
	public function setUp() {
		$this->mockSubject = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
		$this->mockPredicate = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
		$this->mockObject = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
	}
	/**
	 * @test
	 */
	...
	public function equalsCheckTestsSubjectPredicateAndObjectForEquality() {
		...
		$subject1 = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
		$subject2 = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
		$subject1->expects($this->any())->method('equals')->with($subject2)->will($this->returnValue($subjectsAreEqual));

		$predicatesAreEqual = TRUE;
		$predicate1 = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
		$predicate2 = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
		$predicate1->expects($this->any())->method('equals')->with($predicate2)->will($this->returnValue($predicatesAreEqual));

		$objectsAreEqual = TRUE;
		$object1 = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
		$object2 = $this->getMock('F3\Semantic\Domain\Model\Rdf\Concept\RdfNode');
